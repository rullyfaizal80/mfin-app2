    ...
    <div class="report-container">
        ...
        <div style="text-align: center; margin-bottom: 20px;">
            ...
            <p style="margin: 5px 0 0 0; font-size: 12px;">
                Sekolah: <strong>{{ $sekolah }}</strong><br>
                Komponen: <strong>{{ $komponen }}</strong><br>
                Laporan: <strong>Piutang Komponen</strong>
            </p>
        </div>
        ...
        <table class="table-data">
            <thead>
                <tr>
                    ...
                    <th width="17%">Jenis Tunggakan</th>
                    <th width="19%">Nilai Piutang (Rp)</th>
                </tr>
            </thead>
            <tbody>
                @php 
                ...
                @forelse($students as $row)
                    @if($row->piutang > 0)
                        <tr>
                            ...
                            <td align="center">{{ $row->nis }}{{ $row->user_id }}</td>
                            <td>{{ $row->fullname }}</td>
                            ...
                            <td align="right">{{ number_format($row->piutang, 0, ',', '.') }}</td>
                        </tr>
                        @php $grandTotal += $row->piutang; @endphp
                    @endif
                @empty
                    <tr>
                        <td colspan="6" align="center" style="padding: 20px;">Tidak ada data piutang untuk komponen ini.</td>
                    </tr>
                @endforelse
                
                <tr>
                    <td colspan="5" align="right" style="font-size: 14px; font-weight: bold; background-color: #f0f0f0;">TOTAL PIUTANG :</td>
