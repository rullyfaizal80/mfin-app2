    ...
    <div class="report-container">
        ...
        <div style="text-align: center; margin-bottom: 20px;">
            ...
            <p style="margin: 5px 0 0 0; font-size: 12px;">
                Periode: <strong>{{ $awal == 'all' ? 'Semua Waktu' : date('d M Y', strtotime($awal)) }}</strong> s.d <strong>{{ $akhir == 'all' ? 'Semua Waktu' : date('d M Y', strtotime($akhir)) }}</strong><br>
                Jenis Komponen: <strong>{{ $component_name }}</strong>
            </p>
        </div>

        {{-- TABEL PEMASUKAN --}}
        <h4 style="margin-bottom: 5px; font-size: 13px;">I. PEMASUKAN</h4>
        <table class="table-data">
            <thead>
                <tr>
                    ...
                    <th width="20%">Dari / Kepada</th>
                    <th width="30%">Keterangan</th>
                    <th width="18%">Nominal (Rp)</th>
                </tr>
            </thead>
            <tbody>
                @php $totalIn = 0; $noIn = 1; @endphp
                @forelse($results->where('payitem_type', 'income') as $row)
                    @php $nominal = $row->credit > 0 ? $row->credit : $row->debit; $totalIn += $nominal; @endphp
                    <tr>
                        <td align="center">{{ $noIn++ }}</td>
                        <td align="center">{{ date('d/m/Y', strtotime($row->tdate)) }}</td>
                        <td align="center"><strong>{{ $row->ref_no }}</strong></td>
                        <td>{{ $row->parent_payto ?: ($row->payto ?: '-') }}</td>
                        <td>{{ $row->note }}</td>
                        <td align="right">{{ number_format($nominal, 0, ',', '.') }}</td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="6" align="center" style="padding: 10px;">Tidak ada data pemasukan.</td>
                    </tr>
                @endforelse
                
                <tr>
                    <td colspan="5" align="right" style="font-size: 13px; font-weight: bold; background-color: #f0f0f0;">SUB TOTAL PEMASUKAN :</td>
        ...
        {{-- TABEL PENGELUARAN --}}
        <h4 style="margin-bottom: 5px; font-size: 13px;">II. PENGELUARAN</h4>
        <table class="table-data">
            <thead>
                <tr>
                    <th width="5%">No</th>
            ...
            <tbody>
                @php $totalEx = 0; $noEx = 1; @endphp
                @forelse($results->where('payitem_type', 'expense') as $row)
                    @php $nominal = $row->debit > 0 ? $row->debit : $row->credit; $totalEx += $nominal; @endphp
                    <tr>
                        <td align="center">{{ $noEx++ }}</td>
                        <td align="center">{{ date('d/m/Y', strtotime($row->tdate)) }}</td>
                        <td align="center"><strong>{{ $row->ref_no }}</strong></td>
                        <td>{{ $row->parent_payto ?: ($row->payto ?: '-') }}</td>
                        <td>{{ $row->note }}</td>
                ...
                @empty
                    <tr>
                        <td colspan="6" align="center" style="padding: 10px;">Tidak ada data pengeluaran.</td>
                    </tr>
                @endforelse
                
                <tr>
                    <td colspan="5" align="right" style="font-size: 13px; font-weight: bold; background-color: #f0f0f0;">SUB TOTAL PENGELUARAN :</td>
                    <td align="right" style="font-size: 13px; font-weight: bold; background-color: #f0f0f0;">
                        {{ number_format($totalEx, 0, ',', '.') }}
                    </td>
                </tr>
            </tbody>
        </table>
