    ...
    <tbody>
        @forelse($upitems as $index => $item)
            <tr>
                ...
                {{-- Kolom 3: AKUN (Pengecekan COA persis CI2) --}}
                <td class="small">
                ...
                {{-- Kolom 4: TYPE (Type & Repeat) --}}
                <td>
                    {{ $item->payitem_type }}<br>
                    {{ $item->pay_repeat }}
                </td>
                
                {{-- Kolom 5: NILAI (Dengan Input text untuk Update) --}}
                <td class="text-end">
                    <input type="text" name="payvalue_{{ $item->upid }}" class="form-control form-control-sm text-end fw-bold" value="{{ number_format($item->payvalue, 0, ',', '.') }}">
                </td>
                ...
                <td class="text-center">
                    <div class="btn-group">
                        ...
                        <a href="#" class="btn btn-sm btn-danger" title="Delete" onclick="return confirm('Hapus komponen ini?')"><i class="bi bi-trash"></i></a>
                    </div>
                </td>
            </tr>
        @empty
        ...
        @endforelse
                        ...
                        @if($upitems->isNotEmpty())
                            <div class="card-footer text-end bg-white">
                                <button type="submit" name="update_btn" value="1" class="btn btn-primary">
                                    <i class="bi bi-save"></i> Update Semua Nominal
                                </button>
                            </div>
                        @endif
                    </form>
