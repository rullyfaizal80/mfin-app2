...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            ...
            <form action="{{ route('class_list.store') }}" method="POST">
                @csrf
                <div class="row g-3">
                    <div class="col-md-6">
                    ...
                    <div class="col-md-6">
                        ...
                        <select id="cschool_id" name="cschool_id" class="form-select">
                            ...
                            @foreach($dropdowns['schools'] as $item)
                                <option value="{{ $item->id }}" {{ old('cschool_id') == $item->id ? 'selected' : '' }}>{{ $item->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <select id="cyear_id" name="cyear_id" class="form-select" required>
                            ...
                            @foreach($dropdowns['years'] as $item)
                                <option value="{{ $item->id }}" {{ old('cyear_id') == $item->id ? 'selected' : '' }}>{{ $item->title }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-6">
                        ...
                        <select id="csubject_id" name="csubject_id" class="form-select" required>
                            ...
                            @foreach($dropdowns['subjects'] as $item)
                                <option value="{{ $item->id }}" {{ old('csubject_id') == $item->id ? 'selected' : '' }}>{{ $item->title }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-md-4">
                        ...
                        <select id="cgrade_id" name="cgrade_id" class="form-select">
                            ...
                            @foreach($dropdowns['grades'] as $item)
                                <option value="{{ $item->id }}" {{ old('cgrade_id') == $item->id ? 'selected' : '' }}>{{ $item->title }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-4">
                        ...
                        <select id="cgroup_id" name="cgroup_id" class="form-select">
                            ...
                            @foreach($dropdowns['groups'] as $item)
                                <option value="{{ $item->id }}" {{ old('cgroup_id') == $item->id ? 'selected' : '' }}>{{ $item->title }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-4">
                        ...
                        <select id="ctype_id" name="ctype_id" class="form-select" required>
                            ...
                            @foreach($dropdowns['types'] as $item)
                                <option value="{{ $item->id }}" {{ old('ctype_id') == $item->id ? 'selected' : '' }}>{{ $item->title }}</option>
                            @endforeach
                        </select>
                    </div>
                        ...
                        <select id="parent1_id" name="parent1_id" class="form-select">
                            <option value="0">- Pilih Wali Kelas 1 -</option>
                             @foreach($dropdowns['teachers'] as $item)
                                <option value="{{ $item->id }}" {{ old('parent1_id') == $item->id ? 'selected' : '' }}>{{ $item->fullname }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-6">
                        ...
                        <select id="parent2_id" name="parent2_id" class="form-select">
                            <option value="0">- Pilih Wali Kelas 2 -</option>
                             @foreach($dropdowns['teachers'] as $item)
                                <option value="{{ $item->id }}" {{ old('parent2_id') == $item->id ? 'selected' : '' }}>{{ $item->fullname }}</option>
                            @endforeach
                        </select>
                    </div>
