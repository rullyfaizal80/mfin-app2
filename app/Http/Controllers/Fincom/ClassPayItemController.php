...
class ClassPayItemController extends Controller
{
    ...
    {
        // 1. Ambil detail informasi kelas untuk header halaman
        $class_info = DB::table('sis_class_list as cl')
            ...
            ->select('cl.id', 'cl.title as class_title', 'sch.name as school_name', 'cy.title as year_title')
            ->first();
        ...
        $all_classes     = DB::table('sis_class_list')->orderBy('title', 'asc')->get(); // Untuk opsi Copy Data

        // Master COA dipisahkan berdasarkan rumpun Akuntansi (coaclass_id) sesuai bawaan CI2 Anda
